...
export default function LoginForm({ lang, showSignUp = true }: LoginFormProps) {
  ...
  const t = {
    ...
    signUp: lang === "en" ? "Sign up" : "تسجيل جديد",
  };
    ...
    <form onSubmit={onSubmit} className="space-y-6" dir={lang === "ar" ? "rtl" : "ltr"}>
      ...
      {showSignUp ? (
        <div>
          <Link
            href="/register"
            className="flex w-full justify-center rounded-md border border-indigo-200 bg-white py-2 px-4 text-sm font-medium text-indigo-600 shadow-sm hover:bg-indigo-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition-all"
          >
            {t.signUp}
          </Link>
        </div>
