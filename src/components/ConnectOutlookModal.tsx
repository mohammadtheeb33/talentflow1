...
export function ConnectOutlookModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div 
        className="w-full max-w-2xl transform overflow-hidden rounded-xl bg-white shadow-2xl transition-all"
        role="dialog"
        aria-modal="true"
      >
        {/* 1. Header */}
        <div className="flex items-center justify-between border-b border-gray-100 px-6 py-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg font-semibold text-gray-900">Import CVs from Outlook</h3>
              <p className="text-sm text-gray-500">Fetch CV attachments and analyze them automatically</p>
            </div>
          </div>
          <button 
            onClick={onClose} 
        ...
        <div className="px-6 py-6 space-y-8">
            ...
            <div className="flex items-center justify-between rounded-lg border border-gray-200 bg-gray-50/50 p-4">
                ...
                <div className="flex flex-col">
                  ...
                  {connected && (email || displayName) && (
                    <span className="text-xs text-gray-500">
                      Signed in as <span className="font-medium text-gray-700">{email || displayName}</span>
                    </span>
                  )}
                </div>
              </div>
              ...
              {connected ? (
                ...
                <button
                  ...
                  disabled={connecting}
                  className="flex items-center gap-2 rounded-md bg-white border border-gray-300 px-3 py-1.5 text-sm font-medium text-gray-700 shadow-sm hover:bg-gray-50 disabled:opacity-50"
                >
          ...
          <div className="space-y-6">
             ...
             <div className="space-y-3">
               ...
               <div className="relative">
                 <select 
                   ...
                   className="block w-full appearance-none rounded-lg border border-gray-300 bg-white py-2.5 pl-3 pr-10 text-sm text-gray-900 focus:border-indigo-500 focus:ring-indigo-500"
                 >
             ...
             <div className="space-y-3">
               ...
               {showFilters && (
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-4 rounded-lg border border-gray-200 bg-gray-50 p-4 animate-in slide-in-from-top-2 duration-200">
                    <div className="space-y-1.5">
                      ...
                      <div className="flex gap-2">
                        <input 
                          ...
                          className="w-full rounded-md border border-gray-300 p-2 text-xs focus:border-indigo-500 focus:ring-indigo-500" 
                        />
          ...
          <div>
            <button
              ...
              className="w-full flex items-center justify-center gap-2 rounded-lg bg-indigo-600 px-4 py-3 text-sm font-semibold text-white shadow-sm hover:bg-indigo-700 disabled:bg-gray-300 disabled:cursor-not-allowed transition-all"
            >
              {fetching ? (
                <>
          ...
          {(fetching || logs.length > 0) && (
            <div className="rounded-lg border border-gray-200 bg-gray-50 overflow-hidden">
               <div className="bg-gray-100 px-4 py-2 border-b border-gray-200 flex justify-between items-center">
                  <span className="text-xs font-semibold text-gray-700">Status Log</span>
                  {createdCount !== null && (
               ...
               <div className="max-h-32 overflow-y-auto p-4 space-y-1.5 custom-scrollbar">
                  {logs.length === 0 && <p className="text-xs text-gray-500 italic">Waiting to start...</p>}
                  {logs.map((l, i) => (
                    <div key={i} className="flex items-start gap-2 text-xs">
                       ...
                       <span className={l.type === "error" ? "text-red-700 font-medium" : "text-gray-600"}>
                         {l.message}
                       </span>
                    </div>
                  ))}
