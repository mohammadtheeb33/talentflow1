...
export const CandidateRow = memo(function CandidateRow({ item, onDeleted, selected, onSelect, jobTitles }: { item: CandidateItem; onDeleted?: (id: string) => void; selected?: boolean; onSelect?: (checked: boolean) => void; jobTitles?: Record<string, string> }) {
  ...
  function fmtSubmitted(ts?: any) {
    ...
    if (sameDay) {
        return <span className="text-green-600 font-medium">Today <span className="text-gray-500 font-normal">{d.toLocaleTimeString([], { hour: "numeric", minute: "2-digit" })}</span></span>;
    }
    if (isYest) {
        return <span className="text-gray-700">Yesterday <span className="text-gray-500 font-normal">{d.toLocaleTimeString([], { hour: "numeric", minute: "2-digit" })}</span></span>;
    }
    const dow = d.toLocaleDateString([], { weekday: "short" });
    const dateStr = d.toLocaleDateString([], { month: "short", day: "numeric" });
    return <span className="text-gray-600">{dateStr} <span className="text-gray-500 text-xs">({dow})</span></span>;
  }
  ...
  return (
    <tr className={`group border-b border-gray-100 transition-colors hover:bg-gray-50/50 ${selected ? "bg-indigo-50/30" : ""}`}>
      <td className="pl-3 py-2 w-4">
        <input
          type="checkbox"
          aria-label={`Select candidate ${item.parsed?.name || item.name || "Unnamed Candidate"}`}
      ...
      <td className="px-3 py-2">
        {scoreNum !== null ? (
            ...
            <span className="text-xs text-gray-500 italic" title={`Raw: ${JSON.stringify(rawScore)}`}>Not scored</span>
        )}
      </td>
      <td className="px-3 py-2">
      ...
      <td className="px-3 py-2">
        <div className="flex items-center gap-2 opacity-0 transition-opacity group-hover:opacity-100">
            <button
                ...
                className={`rounded-md p-2 hover:bg-green-50 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 ${localStatus === 'accepted' ? 'text-green-600 bg-green-50' : 'text-gray-500 hover:text-green-600'}`}
                title="Accept"
            >
            ...
            <button
                ...
                className={`rounded-md p-2 hover:bg-red-50 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 ${localStatus === 'rejected' ? 'text-red-600 bg-red-50' : 'text-gray-500 hover:text-red-600'}`}
                title="Reject"
            >
            ...
            <button 
                onClick={deleteCv} 
                disabled={deleting}
                className="rounded-md border border-gray-200 bg-white p-1 text-gray-500 hover:bg-red-50 hover:text-red-600 hover:border-red-100 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 disabled:opacity-50"
